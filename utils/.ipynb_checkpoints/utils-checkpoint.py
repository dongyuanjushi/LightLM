...
def exact_match(logger, predictions, targets, scores, total_sequence_generated):
    index = 0
    ...
    for p, s, t in zip(batched_predictions, batched_scores, targets):
        ...
        idx += 1
        t = t.replace("item_", "")
        p = [one_p.replace("item_", "") for one_p in p][:20]
        index = p.index(t) if t in p else -1
        # if idx % 20 == 0:
        #     logger.log("P: {}".format(",".join(p)))
        #     logger.log("T: {}".format(t))
        #     logger.log("Index: {}".format(index))
        #     logger.log("*" * 20)
        #     print()
        if t in p[:5]:
            hit_5 += 1
        ...
        if t in p[:10]:
            ...
            ncdg_10 += 0
        if t in p[:20]:
            gold_position = p[:20].index(
                t
            )
            true_scores = [0.0] * 20
            true_scores[gold_position] = 1.0
            true_scores_20 = np.array([true_scores])
            predict_scores_20 = np.array([s[:20]])
            ncdg_20 += ndcg_score(true_scores_20, predict_scores_20)
        else:
            ncdg_20 += 0
...
def random_initialization(model, tokenizer):
    ...
    for index in ids:
        ...
        # model.shared.weight.data[index] = torch.nn.init.normal_(
        #     model.shared.weight.data[index]
        # )
